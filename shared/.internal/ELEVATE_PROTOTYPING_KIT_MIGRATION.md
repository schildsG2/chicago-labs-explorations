...
- [ ] **1.1** Create `schildsG2/elevate-prototyping-kit` on GitHub (public, same as elevate-lite)
- [ ] **1.2** Seed with elevate-lite content (copy, not fork — clean history)
- [ ] **1.3** Keep `components/elevate.css` as-is from elevate-lite (NOT replacing with npm dist — see Phase 0 decision)
- [ ] **1.4** Author `tokens/elevate.css` as combined file: 59 aliases mapping unprefixed → `--elv-*` vars, plus ~151 hardcoded token values (palette, typography, spacing, radius, shadows) not exposed by elevate-g2
- [ ] **1.5** Add `sync.sh` script
- [ ] **1.6** Update `README.md` to explain the kit's purpose, relationship to @g2crowd/elevate, and sync process
- [ ] **1.7** Remove `.internal/` maintainer docs that are chicago-labs-specific (not kit-specific)
- [ ] **1.8** Remove `components/ELEVATE_COMPONENTS.md` (outdated — the templates themselves are the reference)
- [ ] **1.9** Tag initial commit as `v1.0.0`

**Validation gate** (before proceeding to Phase 2):

- [ ] **1.V1** Create `_test-validation.html` in kit repo root with this content:
  ```html
  <!DOCTYPE html>
  <html lang="en"><head>
  ...
  ```
- [ ] **1.V2** Open `_test-validation.html` in Chrome/Safari. Pass criteria (all must be true):
...
- [ ] **1.V3** Open `components/templates/navigation/vendor-admin-shell.html` in browser. Pass criteria:
...
- [ ] **1.V4** Delete `_test-validation.html` after all checks pass (not committed to repo)

### Phase 2: Update chicago-labs-explorations

**Goal**: New explorations use the kit. Old epics continue working untouched.

- [ ] **2.1** Freeze elevate-lite: convert submodule to regular files
  ```bash
...
- [ ] **2.2** Add new submodule
  ...
  ```
- [ ] **2.3** Update forward-looking files (paths: `shared/elevate-lite` → `shared/elevate-prototyping-kit`):
...
- [ ] **2.5** Commit: `"Update forward-looking files to use elevate-prototyping-kit"`
- [ ] **2.6** Verify Phase 2 (all must pass):
...
- [ ] **3.1** Update submodule placeholder (`shared/elevate-lite-PLACEHOLDER.md` → references elevate-prototyping-kit)
- [ ] **3.2** Update HTML files (paths: `shared/elevate-lite` → `shared/elevate-prototyping-kit`):
  - `index.html`
  - `epics/example-epic/index.html`
  - `epics/example-epic/explorations/01-example.html`
  - `shared/exploration-starter.html`
- [ ] **3.3** Update skills (submodule URL + path references):
...
- [ ] **3.5** Update `CLAUDE.md` — all path references
- [ ] **3.6** Commit: `"Migrate from elevate-lite to elevate-prototyping-kit"`
- [ ] **3.7** Verify Phase 3 (all must pass):
